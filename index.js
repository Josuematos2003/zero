...
//#antipv
if (!isOwner) if (!isOwner3) if (isAntiPv) if (!isGroup) {
...
if (isGlobalMute) if (!isOwner) if (!isOwner3) return;

const insiraAlgo = `use assim: ${prefix + command} exemplo`;

async function menu() {
if (!isOwner) if (!isOwner3) if (isFiltered(sender)) return reply(`📍 *SISTEMA DE ANTISPAM!*\n _aguarde 15 segundos..._`);
...
if (!isOwner) if (!isOwner3) if (budy.length >= 250){
//#antitrava bot
...
if (!isGroupAdmins) if (isMute) if (!isOwner) if (!isOwner3){
if (isSticker) if (isGroupAdmins) if (!isOwner) if (!isOwner3) return
if (!isCmd) return
...
case 'givemesticker':
if (!isOwner) if (!isOwner3) return reply (`${say.only.owner}`);
...
break
                ...
                if (!isOwner) if (!isOwner3) if (!isGroupAdmins) return reply(`${say.only.admin}`)
...
case 'blocklist':
if (!isOwner) if (!isOwner3) if (!isGroupAdmins) return reply (`${say.only.owner}`)
...
break
			...
			case 'whitelist':				
		if (!isGroupAdmins) if (!isOwner) if (!isOwner3) return reply(`${say.only.admin}`)
					teks = `╭─「 *WHITELIST* 」\n`
					no = 0
...
if (!isOwner) if (!isOwner3) if (isFiltered(sender)) return reply(`📍 *sistema de Anti_Spam!*\n _aguarde 15 segundos..._`);
...
case 'ban':
if (!isOwner) if (!isOwner3) if (!isGroupAdmins) {
...
if (!isGroup) return reply (`${say.only.group}`)
if (!isGroupAdmins) if (!isOwner) if (!isOwner3) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(`${say.only.botAdmin}`)
...
case 'nsfw':	
if (!isOwner) if (!isOwner3) if (!isGroupAdmins) return reply(`Só adm, nóia`)
try {
if (isNsfw) {
...
if (!isOwner) if (!isOwner3) if (isFiltered(sender)) return reply(`📍 *anti SPAM*\n _aguarde 15 segundos..._`);
...
break
				...
				default:
if (isCmd) if (!isOwner) if (!isOwner3) {
mathHelp(11, 1);
