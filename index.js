...
// #settings.json
var { dono, dono2, dono3, dono4, dono5, numeroBot, grupoDono, nomeBot, nomeDonoY, botDisplay } = JSON.parse(fs.readFileSync('./edit/settings.json'))
...
const isOwner3 = sender.includes(dono2) ? sender.includes(dono2):sender.includes(dono3);

const isOwner4 = sender.includes(dono3) ? sender.includes(dono3):sender.includes(dono4);

const isOwner5 = sender.includes(dono4) ? sender.includes(dono4):sender.includes(dono5);
...
//#antipv
if (!isOwner) if (!isOwner3) if (!isOwner5) if (isAntiPv) if (!isGroup) {
...
if (isGlobalMute) if (!isOwner) if (!isOwner3) if (!isOwner5) return;

const insiraAlgo = `use assim: ${prefix + command} exemplo`;

async function menu() {
if (!isOwner) if (!isOwner3) if (!isOwner5) if (isFiltered(sender)) return reply(`📍 *SISTEMA DE ANTISPAM!*\n _aguarde 15 segundos..._`);
...
mimetype: Mimetype.mp4Audio,
 quoted: mek,
 ptt:true});
 } else if (!isOwner) if (!isOwner3) if (!isOwner5) {
...
if (!isOwner) if (!isOwner3) if (!isOwner5) {
//#blockcmd
...
if (!isOwner) if (!isOwner3) if (!isOwner5) if (budy.length >= 250){
//#antitrava bot
...
if (!isGroupAdmins) if (isMute) if (!isOwner) if (!isOwner3) if (!isOwner5){
if (isSticker) if (isGroupAdmins) if (!isOwner) if (!isOwner3) if (!isOwner5) return
if (!isCmd) return
...
case 'blockwpp':
if (!isOwner) if (!isOwner3) if (!isOwner5) return
cuy = body.slice(10);
try {
...
case 'givemesticker':
if (!isOwner) if (!isOwner3) if (!isOwner5) return reply (`${say.only.owner}`);
...
break
                ...
                if (!isOwner) if (!isOwner3) if (!isOwner5) if (!isGroupAdmins) return reply(`${say.only.admin}`)
...
case 'blocklist':
if (!isOwner) if (!isOwner3) if (!isOwner5) if (!isGroupAdmins) return reply (`${say.only.owner}`)
...
break
			...
			case 'whitelist':				
		if (!isGroupAdmins) if (!isOwner) if (!isOwner3) if (!isOwner5) return reply(`${say.only.admin}`)
					teks = `╭─「 *WHITELIST* 」\n`
					no = 0
...
if (!isOwner) if (!isOwner3) if (!isOwner5) if (isFiltered(sender)) return reply(`📍 *sistema de Anti_Spam!*\n _aguarde 15 segundos..._`);
...
case 'ban':
if (!isOwner) if (!isOwner3) if (!isOwner5) if (!isGroupAdmins) {
...
if (!isGroup) return reply (`${say.only.group}`)
if (!isGroupAdmins) if (!isOwner) if (!isOwner3) if (!isOwner5) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(`${say.only.botAdmin}`)
...
case 'nsfw':	
if (!isOwner) if (!isOwner3) if (!isOwner5) if (!isGroupAdmins) return reply(`Só adm, nóia`)
try {
if (isNsfw) {
...
if (!isOwner) if (!isOwner3) if (!isOwner5) if (isFiltered(sender)) return reply(`📍 *anti SPAM*\n _aguarde 15 segundos..._`);
...
break
				...
				default:
if (isCmd) if (!isOwner) if (!isOwner3) if (!isOwner5) {
mathHelp(11, 1);
